...
Serve.checkForDocumentRoot = function checkForDocumentRoot(options) {
  if (!fs.existsSync( path.join(options.appDirectory, options.documentRoot) )) {
    if (options.createDocumentRoot) {
      ...
      events.emit('log', '"' + options.documentRoot + '" directory cannot be found. Please make sure the working directory is an Ionic project.');
      return false;
...
Serve.runLivereload = function runLivereload(options, app) {
  ...
  try {
    ...
    // events.emit('verbose', 'Absolute watch paths:', absoluteWatchPaths);
    ...
    q.reject(ex);
    events.emit('log', 'Live Reload failed to start, error: ', ex.message);
    throw ex;
